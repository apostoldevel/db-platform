...
**db-platform** is a PL/pgSQL framework (27 modules, 100+ tables, 800+ functions) that turns PostgreSQL into a full-featured application server: REST API, OAuth2, workflow engine, entity system, file storage, pub/sub, reports. Version: see `VERSION` file.
...
Defined in `create.psql` and `update.psql`. 27 modules in dependency order:
kernel → oauth2 → locale → admin → http → resource → exception → error → registry → log → api → replication → daemon → session → current → workflow → kladr → file → entity → mq → notice → comment → notification → verification → observer → report → reports
