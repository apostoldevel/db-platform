...
-- ERR-403-010: RouteNotGranted. A /api/v2 route refused by its guard (daemon.begin), or one with no guard: the gateway is closed by default. Added in 1.2.31.
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'en', 'Access to %s %s is not granted', 'The route is not open to this session: its guard refused, or no guard is registered for it.', 'Use an account allowed on this route, or ask an administrator.');
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'ru', 'Доступ к %s %s не предоставлен');
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'de', 'Zugriff auf %s %s ist nicht gewährt', 'Die Route ist für diese Sitzung nicht geöffnet: ihr Wächter hat abgelehnt, oder für sie ist kein Wächter registriert.', 'Verwenden Sie ein Konto, das für diese Route zugelassen ist, oder wenden Sie sich an einen Administrator.');
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'fr', 'L''accès à %s %s n''est pas accordé', 'La route n''est pas ouverte à cette session : son garde a refusé, ou aucun garde n''est enregistré pour elle.', 'Utilisez un compte autorisé sur cette route, ou demandez à un administrateur.');
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'it', 'L''accesso a %s %s non è consentito', 'Il percorso non è aperto a questa sessione: la sua guardia ha rifiutato, oppure non è registrata alcuna guardia.', 'Usare un account autorizzato su questo percorso o rivolgersi a un amministratore.');
SELECT RegisterError('ERR-403-010', 403, 'E', 'access', 'es', 'El acceso a %s %s no está concedido', 'La ruta no está abierta a esta sesión: su guarda la rechazó o no tiene guarda registrada.', 'Use una cuenta permitida en esta ruta o consulte a un administrador.');

-- ERR-403-011: GatewayRequestNotOpen. daemon.call or daemon.end without a successful daemon.begin in the same transaction. Added in 1.2.31.
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'en', 'No open request: call daemon.begin first', 'The gateway call has no request opened by daemon.begin in the same transaction, so there is no verified session to act for.', 'Open the request with daemon.begin in the same transaction and check that it was authorized.');
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'ru', 'Нет открытого запроса: сначала вызовите daemon.begin');
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'de', 'Keine offene Anfrage: zuerst daemon.begin aufrufen', 'Für den Gateway-Aufruf wurde in derselben Transaktion keine Anfrage mit daemon.begin geöffnet, daher gibt es keine geprüfte Sitzung.', 'Öffnen Sie die Anfrage mit daemon.begin in derselben Transaktion und prüfen Sie, dass sie autorisiert wurde.');
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'fr', 'Aucune requête ouverte : appelez d''abord daemon.begin', 'L''appel de passerelle n''a aucune requête ouverte par daemon.begin dans la même transaction, il n''y a donc pas de session vérifiée.', 'Ouvrez la requête avec daemon.begin dans la même transaction et vérifiez qu''elle a été autorisée.');
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'it', 'Nessuna richiesta aperta: chiamare prima daemon.begin', 'La chiamata al gateway non ha una richiesta aperta da daemon.begin nella stessa transazione, quindi non c''è una sessione verificata.', 'Aprire la richiesta con daemon.begin nella stessa transazione e verificare che sia stata autorizzata.');
SELECT RegisterError('ERR-403-011', 403, 'E', 'access', 'es', 'No hay solicitud abierta: llame primero a daemon.begin', 'La llamada a la pasarela no tiene una solicitud abierta por daemon.begin en la misma transacción, por lo que no hay sesión verificada.', 'Abra la solicitud con daemon.begin en la misma transacción y compruebe que fue autorizada.');

-- ERR-403-012: GatewayFunctionNotOpen. daemon.call for a function not in the gateway allow list (gateway.function), or one the passed keys do not select. Added in 1.2.31.
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'en', 'Function "%s" is not open to the gateway', 'The function is not in the gateway allow list, or the keys passed select none of its registered forms.', 'Register the function with RegisterGatewayFunction, or pass the keys of a registered form.');
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'ru', 'Функция "%s" не открыта для шлюза');
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'de', 'Funktion "%s" ist für das Gateway nicht geöffnet', 'Die Funktion steht nicht in der Gateway-Freigabeliste, oder die übergebenen Schlüssel wählen keine ihrer registrierten Formen.', 'Registrieren Sie die Funktion mit RegisterGatewayFunction oder übergeben Sie die Schlüssel einer registrierten Form.');
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'fr', 'La fonction "%s" n''est pas ouverte à la passerelle', 'La fonction n''est pas dans la liste autorisée de la passerelle, ou les clés passées ne sélectionnent aucune de ses formes enregistrées.', 'Enregistrez la fonction avec RegisterGatewayFunction, ou passez les clés d''une forme enregistrée.');
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'it', 'La funzione "%s" non è aperta al gateway', 'La funzione non è nell''elenco consentito del gateway, oppure le chiavi passate non selezionano nessuna delle sue forme registrate.', 'Registrare la funzione con RegisterGatewayFunction o passare le chiavi di una forma registrata.');
SELECT RegisterError('ERR-403-012', 403, 'E', 'access', 'es', 'La función "%s" no está abierta a la pasarela', 'La función no está en la lista permitida de la pasarela, o las claves pasadas no seleccionan ninguna de sus formas registradas.', 'Registre la función con RegisterGatewayFunction o pase las claves de una forma registrada.');

-- ERR-403-013: GatewayFunctionAdminOnly. daemon.call of a function registered at level administrator, under a session that is not an administrator. Added in 1.2.31.
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'en', 'Function "%s" requires an administrator', 'The function is open to the gateway at the administrator level only.', 'Use an administrator account.');
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'ru', 'Функция "%s" доступна только администратору');
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'de', 'Funktion "%s" erfordert einen Administrator', 'Die Funktion ist für das Gateway nur auf Administratorebene geöffnet.', 'Verwenden Sie ein Administratorkonto.');
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'fr', 'La fonction "%s" requiert un administrateur', 'La fonction n''est ouverte à la passerelle qu''au niveau administrateur.', 'Utilisez un compte administrateur.');
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'it', 'La funzione "%s" richiede un amministratore', 'La funzione è aperta al gateway solo a livello di amministratore.', 'Usare un account amministratore.');
SELECT RegisterError('ERR-403-013', 403, 'E', 'access', 'es', 'La función "%s" requiere un administrador', 'La función solo está abierta a la pasarela en el nivel de administrador.', 'Use una cuenta de administrador.');

-- ERR-403-014: ProtectedGroupError. A member included into or excluded from a protected group (administrator, system, … — db.protected_group) by a session that is not an administrator. Added in 1.2.31.
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'en', 'Group "%s" is protected: only an administrator may change its members', 'Membership of this group grants rights by itself, so only an administrator may include or exclude its members.', 'Ask an administrator.');
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'ru', 'Группа "%s" защищена: менять её состав может только администратор');
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'de', 'Gruppe "%s" ist geschützt: nur ein Administrator darf ihre Mitglieder ändern', 'Die Mitgliedschaft in dieser Gruppe gewährt selbst Rechte, daher darf nur ein Administrator Mitglieder aufnehmen oder entfernen.', 'Wenden Sie sich an einen Administrator.');
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'fr', 'Le groupe "%s" est protégé : seul un administrateur peut modifier ses membres', 'L''appartenance à ce groupe accorde des droits par elle-même, seul un administrateur peut donc y inclure ou en exclure des membres.', 'Demandez à un administrateur.');
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'it', 'Il gruppo "%s" è protetto: solo un amministratore può modificarne i membri', 'L''appartenenza a questo gruppo concede diritti di per sé, quindi solo un amministratore può includere o escludere membri.', 'Rivolgersi a un amministratore.');
SELECT RegisterError('ERR-403-014', 403, 'E', 'access', 'es', 'El grupo "%s" está protegido: solo un administrador puede cambiar sus miembros', 'Pertenecer a este grupo concede derechos por sí mismo, por lo que solo un administrador puede incluir o excluir miembros.', 'Consulte a un administrador.');

-- ERR-403-015: GroupExceedsRightsError. A member included into or excluded from a group whose ACL grants rights the caller does not have. Added in 1.2.31.
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'en', 'Group "%s" grants rights you do not have', 'The group''s access control list allows operations your own does not, so changing its members would raise someone above you.', 'Ask a user who holds those rights, or an administrator.');
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'ru', 'Группа "%s" даёт права, которых у вас нет');
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'de', 'Gruppe "%s" gewährt Rechte, die Sie nicht haben', 'Die Zugriffsliste der Gruppe erlaubt Vorgänge, die Ihre eigene nicht erlaubt; eine Änderung der Mitglieder würde jemanden über Sie stellen.', 'Wenden Sie sich an einen Benutzer mit diesen Rechten oder an einen Administrator.');
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'fr', 'Le groupe "%s" accorde des droits que vous n''avez pas', 'La liste de contrôle d''accès du groupe autorise des opérations que la vôtre n''autorise pas ; modifier ses membres élèverait quelqu''un au-dessus de vous.', 'Demandez à un utilisateur disposant de ces droits, ou à un administrateur.');
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'it', 'Il gruppo "%s" concede diritti che non possiedi', 'L''elenco di controllo degli accessi del gruppo consente operazioni che il tuo non consente; modificarne i membri innalzerebbe qualcuno sopra di te.', 'Rivolgersi a un utente con quei diritti o a un amministratore.');
SELECT RegisterError('ERR-403-015', 403, 'E', 'access', 'es', 'El grupo "%s" concede derechos que usted no tiene', 'La lista de control de acceso del grupo permite operaciones que la suya no; cambiar sus miembros elevaría a alguien por encima de usted.', 'Consulte a un usuario con esos derechos o a un administrador.');
