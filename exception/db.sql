...
SELECT CreateExceptionResource(GetExceptionUUID(400, 7), 'ru', 'InvalidArea', 'Некоторые из запрошенных областей недействительны: {верные=[%s], неверные=[%s]}');
SELECT CreateExceptionResource(GetExceptionUUID(400, 7), 'en', 'InvalidArea', 'Some requested areas were invalid: {valid = [%s], invalid = [%s]}');

CREATE OR REPLACE FUNCTION InvalidArea (
  pValid    text[],
  pInvalid  text[]
) RETURNS	void
AS $$
