...
BEGIN
  ...
  nDocument := CreateDocument(pParent, pType, pSubject, pDescription);
...
BEGIN
  ...
  IF pSubject <> coalesce(cSubject, '<null>') THEN
    UPDATE db.object SET label = CheckNull(pSubject) WHERE id = pId;
  END IF;
