...
--------------------------------------------------------------------------------
/**
 * Запрос данных в формате REST JSON API (Задача).
 * @param {text} pPath - Путь
 * @param {jsonb} pPayload - JSON
 * @return {SETOF json} - Записи в JSON
 */
CREATE OR REPLACE FUNCTION rest.task (
  pPath       text,
...
BEGIN
  ...
  WHEN '/task/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEssence('task')
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;

  WHEN '/task/method' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT r.id, api.get_methods(GetObjectClass(r.id), GetObjectState(r.id)) as method FROM api.get_task(r.id) ORDER BY id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/task/set' THEN
    ...
    arKeys := array_cat(arKeys, GetRoutines('set_task', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN EXECUTE format('SELECT row_to_json(api.set_task(%s)) FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_task', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_task', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT row_to_json(api.set_task(%s)) FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_task', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_task', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
  ...
  WHEN '/task/get' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_task($1)', JsonbToFields(r.fields, GetColumns('task', 'api'))) USING r.id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
