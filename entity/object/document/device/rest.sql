...
--------------------------------------------------------------------------------
/**
 * Запрос данных в формате REST JSON API (Устройство).
...
BEGIN
  ...
  WHEN '/device/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEssence('device')
      LOOP
  ...
  WHEN '/device/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT count(*) FROM api.list_device(r.search, r.filter, r.reclimit, r.recoffset, r.orderby)
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
