...
--------------------------------------------------------------------------------
/**
 * @brief Dispatch REST JSON API requests for the Reference entity.
 * @param {text} pPath - Route path (e.g., /reference/get, /reference/list)
 ...
 * @since 1.0.0
 */
CREATE OR REPLACE FUNCTION rest.reference (
  pPath       text,
...
BEGIN
  ...
  WHEN '/reference/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT count(*) FROM api.list_reference(r.search, r.filter, r.reclimit, r.recoffset, r.orderby)
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/reference/set' THEN

    IF pPayload IS NULL THEN
      PERFORM JsonIsEmpty();
    END IF;

    arKeys := array_cat(arKeys, GetRoutines('set_reference', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN EXECUTE format('SELECT row_to_json(api.set_reference(%s)) FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_reference', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_reference', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT row_to_json(api.set_reference(%s)) FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_reference', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_reference', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
  ...
  WHEN '/reference/get' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(id uuid, fields jsonb)
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_reference($1)', JsonbToFields(r.fields, GetColumns('reference', 'api'))) USING r.id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/reference/list' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.list_reference($1, $2, $3, $4, $5)', JsonbToFields(r.fields, GetColumns('reference', 'api'))) USING r.search, r.filter, r.reclimit, r.recoffset, r.orderby
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;
