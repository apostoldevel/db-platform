...
BEGIN
  ...
  WHEN '/vendor/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEntity('vendor')
      LOOP
  ...
  WHEN '/vendor/method' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT r.id, api.get_methods(GetObjectClass(r.id), GetObjectState(r.id)) as method FROM api.get_vendor(r.id) ORDER BY id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
