...
BEGIN
  ...
  WHEN '/form/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEntity('form')
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;

  WHEN '/form/method' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT r.id, api.get_methods(GetObjectClass(r.id), GetObjectState(r.id)) as method FROM api.get_form(r.id) ORDER BY id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/form/set' THEN
    ...
    arKeys := array_cat(arKeys, GetRoutines('set_form', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN EXECUTE format('SELECT row_to_json(api.set_form(%s)) FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_form', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_form', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT row_to_json(api.set_form(%s)) FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_form', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_form', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
  ...
  WHEN '/form/get' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_form($1)', JsonbToFields(r.fields, GetColumns('form', 'api'))) USING r.id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/form/list' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.list_form($1, $2, $3, $4, $5)', JsonbToFields(r.fields, GetColumns('form', 'api'))) USING r.search, r.filter, r.reclimit, r.recoffset, r.orderby
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;

  WHEN '/form/build' THEN
	...
	LOOP
      RETURN NEXT api.build_form(r.id, r.params);
	END LOOP;

  ELSE

    RETURN NEXT rest.form_field(pPath, pPayload);
