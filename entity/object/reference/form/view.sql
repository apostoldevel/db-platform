...
AS
  WITH access AS (
    SELECT * FROM AccessObjectUser(GetEntity('form'), current_userid())
  )
  SELECT f.* FROM db.form f INNER JOIN access a ON f.id = a.object;
...
AS
  SELECT t.id, r.object, o.parent,
    ...
    FROM db.form t INNER JOIN db.reference         r ON t.reference = r.id AND r.scope = current_scope()
