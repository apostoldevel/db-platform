...
--------------------------------------------------------------------------------
/**
 * Форма отчёта: Список сессий
 * @param {uuid} pForm - Идентификатор формы
 * @param {jsonb} pParams - Параметры
 * @return {SETOF json} - Записи в JSON
 */
CREATE OR REPLACE FUNCTION report.rfc_session_list (
  pForm         uuid,
 ...
 */
CREATE OR REPLACE FUNCTION report.rpc_session_list (
  pReady        uuid,
...
BEGIN
  ...
  LOOP
    ...
    vCSV := E'session;username;name;email;phone;description;created;updated;host;status\r\n';
	...
	IF l.code = 'ru' THEN
      ...
      vHTML := vHTML || E'        <th style="width: 10%!important;">Сессия</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Пользователь</th>\n';
      ...
      vHTML := vHTML || E'        <th style="width: 10%!important;">Дата создания</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Дата обновления</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Хост</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Статус</th>\n';
      vHTML := vHTML || E'      </tr>\n';
	ELSE
      vHTML := vHTML || E'      <tr class="text-center">\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Session</th>\n';
      ...
      vHTML := vHTML || E'        <th style="width: 10%!important;">Created</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Updated</th>\n';
      vHTML := vHTML || E'        <th style="width: 10%!important;">Host</th>\n';
    ...
    IF uGroupId IS NULL THEN
	  FOR t IN
		SELECT SubStr(s.code, 1, 8) || '...' || SubStr(s.code, 32, 8) AS code, s.host, s.created, s.updated, u.status, u.username, u.name, u.email, u.email_verified, u.phone, u.phone_verified, u.statustext, u,description
		  FROM db.session s INNER JOIN users u ON s.userid = u.id AND u.scope = current_scope()
		 WHERE u.id = coalesce(uUserId, u.id)
	  ...
	  LOOP
		...
		vHTML := vHTML || format(E'          <td style="width: 10%%!important;">%s</td>\n', DateToStr(t.updated, vFormat));
		vHTML := vHTML || format(E'          <td style="width: 10%%!important;">%s</td>\n', t.host);
		vHTML := vHTML || format(E'          <td style="width: 10%%!important;">%s</td>\n', t.statustext);

		vHTML := vHTML || E'        </tr>\n';

		vCSV := vCSV || format(E'%s;', t.code);
		...
		vCSV := vCSV || format(E'%s;', DateToStr(t.updated, vFormat));
		vCSV := vCSV || format(E'%s;', t.host);
	...
	ELSE
	  FOR t IN
		SELECT SubStr(s.code, 1, 8) || '...' || SubStr(s.code, 32, 8) AS code, s.host, s.created, s.updated, u.status, u.username, u.name, u.email, u.email_verified, u.phone, u.phone_verified, u.statustext, u,description
		  FROM db.session s INNER JOIN users u ON s.userid = u.id AND u.scope = current_scope()
		                    INNER JOIN db.member_group mg ON u.id = mg.member AND mg.userid = coalesce(uGroupId, mg.userid)
		 WHERE u.id = coalesce(uUserId, u.id)
		   AND u.status & nStatus != 0
		 ORDER BY u.name
	     LIMIT 500
  ...
  PERFORM SetObjectFile(pReady, format('session_%s.csv', DateToStr(Now(), 'YYYYMMDD_HH24MISS')), null, length(csv_file), localtimestamp, csv_file, encode(digest(csv_file, 'md5'), 'hex'), Lines[1], 'data:text/plain;base64,');
