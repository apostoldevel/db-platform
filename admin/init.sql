...
INSERT INTO db.area_type (id, code, name) VALUES ('00000000-0000-4002-a000-000000000003', 'all', 'Все');
...
INSERT INTO db.area_type (id, code, name) VALUES ('00000000-0000-4002-a001-000000000002', 'subsidiary', 'Дочерний');
...
SELECT CreateArea('00000000-0000-4003-a001-000000000000', GetArea('root'), GetAreaType('all'), GetScope(current_database()), 'all', 'Все');
...
--SELECT CreateArea('00000000-0000-4003-a001-000000000003', GetArea('main'), GetAreaType('subsidiary'), GetScope(current_database()), 'subsidiary', 'Дочернее предприятие');
--SELECT CreateArea('00000000-0000-4003-a001-000000000004', GetArea('subsidiary'), GetAreaType('mobile'), GetScope(current_database()), 'mobile', 'Мобильное подразделение');
...
SELECT AddMemberToGroup(CreateUser('admin', current_setting('password.admin'), 'Администратор', null,null, 'Администратор системы', true, false, '00000000-0000-4000-a001-000000000001'), GetGroup('administrator'));
SELECT CreateUser('daemon', current_setting('password.daemon'), 'Демон', null, null, 'Пользователь для вызова методов API', false, true, '00000000-0000-4000-a001-000000000002');

SELECT AddMemberToGroup(CreateUser('apibot', current_setting('password.apibot'), 'API клиент', null, null, 'Системная служба API', false, true, '00000000-0000-4000-a002-000000000001'), GetGroup('system'));
SELECT CreateUser('mailbot', current_setting('password.mailbot'), 'Mail клиент', null, null, 'Почтовый клиент', false, true, '00000000-0000-4000-a002-000000000002');
