...
BEGIN
  ...
  WHEN '/admin/session/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_session(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/event/log' THEN
    ...
    FOR r IN SELECT * FROM jsonb_to_record(pPayload) AS x(username text, type char, code integer, datefrom timestamptz, dateto timestamptz)
    LOOP
  ...
  WHEN '/admin/event/log/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_event_log(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/api/log/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_log(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/user/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_user(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/group/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_group(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/area/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_area(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/admin/interface/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT * FROM api.count_interface(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
