...
#### Общие
```http request
POST /api/v1/task/[ type | method | count | set | get | list ]
```
[Общие конечные точки](#общие-конечные-точки)
...
type | STRING |  | **Обязательный**. Код типа.
code | STRING |  | **Обязательный**. Код.
label | STRING |  | **Обязательный**. Метка.
calendar | NUMERIC |  | **Необязательный**. Календарь.
scheduler | NUMERIC |  | **Необязательный**. Планировщик.
program | NUMERIC |  | **Необязательный**. Программа.
executor | NUMERIC |  | **Необязательный**. Исполнитель.
